...
- **Otherwise** → **`tool:create_view`**, then iterate with **`tool:update_view_pivots`** (placement, calculated items, custom display, styling) and the other `update_view_*` tools (filters, sorts, chart config, aggregations, View formatting, ...). When `tool:create_view` is called with `pivotLayout` null, the server picks a sensible default layout; to override, send a complete `pivotLayout` with all three axes populated (empty array = no pivot on that axis). **Editing the live View behind a board widget** → call the matching `update_view_*` tool directly on that View id — if a Draft was created, pair the edit with **`tool:set_widget_preview`** so this user sees the Draft on the widget (see [view_widgets.md](../designing-boards/view_widgets.md)).
